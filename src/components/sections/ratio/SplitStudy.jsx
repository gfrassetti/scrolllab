...
export default function SplitStudy({
  kicker = 'Lorem ipsum dolor sit',
  meta = 'Studio — 2024',
  index = '01',
  specLabel = 'lorem',
  specValue = 'ipsum',
  spec2Label = 'lorem - ipsum',
  spec2Value = 'dolor sit',
  spec3Label = 'col',
  spec3Value = 'amet',
  spec4Label = 'lorem',
  spec4Value = 'ipsum',
  ...
  quoteBy = 'A. Sample',
  quoteRole = 'Founder of the placeholder',
  panel2Label = 'Panel',
  ...
  return (
    <section ref={root} id={anchor} className="relative bg-white text-[#111]">
      <div className="flex flex-col lg:flex-row">
        <aside className="lg:sticky lg:top-12 lg:h-[calc(100svh-3rem)] lg:w-[38%] lg:shrink-0">
          <div className="flex h-full flex-col bg-white px-5 py-8 lg:border-r lg:border-[#111] lg:px-8 lg:py-10">
            <header className="flex items-start justify-between gap-4 border-b border-[#111] pb-4">
              <div>
                <p className="font-grotesk text-[12px] tracking-[0.04em]">
                  <span className="mr-2">●</span>
                  {kicker}
                </p>
                <p className="mt-2 font-grotesk text-[12px] tracking-[0.04em] opacity-50">{meta}</p>
              </div>
              <span className="font-grotesk text-[12px] tracking-[0.08em] opacity-50">({index})</span>
            </header>
            <dl className="mt-1">
              {specs.map(([label, value], i) => (
                <div
                  key={`${label}-${i}`}
                  className="flex items-baseline justify-between gap-4 border-b border-[#111] py-3 font-grotesk text-[13px] tracking-[0.03em]"
                >
        ...
        <div className="min-w-0 flex-1 bg-[#111] text-white">
          <article className="flex min-h-[100svh] flex-col justify-center px-4 py-10 md:px-8 lg:px-10">
            <div className="grid min-h-[72svh] bg-white text-[#111] md:grid-cols-2">
              <div className="flex flex-col justify-between gap-10 px-5 py-8 md:px-9 md:py-10">
                ...
                <p className="flex items-center gap-3 font-grotesk text-[11px] tracking-[0.08em] uppercase">
                  <span aria-hidden="true" className="size-9 shrink-0 overflow-hidden rounded-full bg-[#111]">
          ...
          <article className="flex min-h-[100svh] flex-col px-4 py-10 md:px-8 lg:px-10">
            <p className="mb-3 flex items-baseline justify-between font-grotesk text-[11px] tracking-[0.14em] uppercase opacity-65">
