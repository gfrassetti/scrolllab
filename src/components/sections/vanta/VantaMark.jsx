...
/**
 * Geometric VANTA wordmark. The first A is ink over a hole in BootVanta's
 * cream veil — the hero underneath is the same photo, never a second crop.
 */
export default function VantaMark({ progress = 1, ink = 1, className = '' }) {
  const melt = Math.max(0, 1 - progress)
  const uid = 'vanta-mark'

  return (
    <svg viewBox="-24 -28 968 280" className={className} aria-hidden="true">
      <defs>
      ...
      <g filter={melt > 0.02 ? `url(#${uid}-melt)` : undefined}>
        <path
          ...
          fillOpacity={ink}
        />
        <rect data-vanta-a="" x="180" y="18" width="166" height="204" fill="#000" fillOpacity="0" />
        ...
        <rect data-vanta-letter="" x="880" y="96" width={48 + melt * 80} height="28" fill="#111114" />
      </g>

      <g fill="#111114" opacity={melt * 0.85} data-vanta-letter="">
        {BLOBS.map(([x, y, w, h], i) => (
          <rect
            key={i}
            x={x}
